...
const STATUS_COLORS: Record<string, string> = {
  ...
  running: "border-agent-running",
  done: "border-agent-done",
  failed: "border-agent-failed",
  paused: "border-agent-paused ring-2 ring-agent-paused/30",
};
...
export function HumanNode({ data, selected }: NodeProps) {
  ...
  return (
    ...
    >
      ...
      <div className="text-xs uppercase tracking-wide text-amber-600">HUMAN</div>
      <div className="font-medium text-sm">{d.name}</div>
      <div className="text-[11px] text-slate-500">awaits user input</div>
